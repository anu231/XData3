...
public class CountEstimationRelated {
	...
	public static boolean isAssignmentSatisfiable(GenerateCVC1 cvc, QueryBlockDetails queryBlock) throws Exception{
		...
		while(itr.hasNext()){
			...
			String cmdString = "";
			cmdString = "#!/bin/bash\n";
			cmdString += "cvc3 "+ Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/checkAggConstraints.cvc | grep -e 'Invalid' > isValid \n";
			...
			System.out.println("sh "+ Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/checkAggConstraints");
		...
		if(havingClauseConstraints.size() != 0){ 
			...
			while(itr.hasNext()){
				...
				for(;;c++){
					...
					cmdString = "";
					cmdString = "#!/bin/bash\n";
					cmdString += "cvc3 "+ Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/getCount.cvc > "+Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/COUNTCVC \n";
					cmdString += "cvc3 "+ Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/getCount.cvc | grep -e 'Invalid' > isValid \n";
					cmdString += "grep -e 'COUNT = 0bin' "+ Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/COUNTCVC" +" | awk -F \" \" '{print $4}' | awk -F \")\" '{print $1}' | awk -F \"bin\" '{print $2}' > "+Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/COUNT\n";
					...
					BufferedReader input =  new BufferedReader(new FileReader(Configuration.homeDir+"/temp_cvc" + cvc.getFilePath() + "/COUNT"));
					String countFromFile = input.readLine();
					
					writer.close();
					myIStreamReader.close();
					input.close();
					countVal = Integer.parseInt(countFromFile,2);
					countVal = c;
				...
				System.out.println("# of CVC calls: "+c);
			...
			System.out.println("Estimated finalCount: " + finalCount);
			return finalCount;
		}	
		else 
