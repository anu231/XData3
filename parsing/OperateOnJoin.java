...
public class OperateOnJoin {
	...
	public static FromListElement OperateOnJoinNodeJSQL(Join joinNode,
		...
		if (joinNode.isLeft()) { // checks if join is
			joinType = JoinClauseInfo.leftOuterJoin;
		} else if (joinNode.isRight()){
			joinType = JoinClauseInfo.rightOuterJoin;
		} else if (joinNode.isInner()){
			joinType = JoinClauseInfo.innerJoin; 
		}
		...
		if (joinNode.getRightItem() instanceof Join) {
			FromListElement temp = new FromListElement();

			temp = OperateOnJoinNodeJSQL((Join) joinNode.getRightItem(),
